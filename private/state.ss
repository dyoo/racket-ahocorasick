...
(provide make
         ...
         fail
         output
         (rename-out [set-State-output! set-output!])
         add-output!
         (rename-out [set-State-fail! set-fail!])
         extend
         extend*
         out-labels
         out-states
...
(define-struct State (depth edges fail output) #:mutable)
